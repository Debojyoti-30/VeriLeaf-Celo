...
export type PrismaticBurstProps = {
  ...
  // Cap rendering FPS to reduce CPU/GPU usage (default 30)
  maxFps?: number;
};
...
const PrismaticBurst = ({
  ...
  // Choose DPR based on container size to avoid huge canvas pixel counts on large screens
  const w0 = container.clientWidth || 1;
  const h0 = container.clientHeight || 1;
  const baseDpr = Math.min(window.devicePixelRatio || 1, 2);
  // If canvas would be very large, cap DPR to 1 to reduce GPU load
  const area = w0 * h0;
  const dpr = area > 1200 * 800 ? Math.min(baseDpr, 1) : baseDpr;
  ...
  program.uniforms.uMouse.value = [sm[0], sm[1]];
      ...
      if (now - lastRenderTime >= minFrameIntervalRef.current) {
        try {
          ...
          console.warn('PrismaticBurst render error', e);
        }
        lastRenderTime = now;
      }
